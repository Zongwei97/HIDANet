...
class Bottle2neck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, baseWidth=26, scale = 4, stype='normal', k_size=3):
        """ Constructor
        Args:
...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 4, num_classes=1000, k_size=[3, 3, 3, 3]):
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks, k_size, stride=1):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample=downsample, 
                        stype='stage', baseWidth = self.baseWidth, scale=self.scale, k_size = k_size))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, baseWidth = self.baseWidth, scale=self.scale, k_size=k_size))
...
def res2net50_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_v1b_26w_4s'],map_location='cpu'), strict = False)
    return model
...
def res2net101_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net101_v1b_26w_4s']), strict = False)
    return model
...
def res2net152_v1b_26w_4s(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net152_v1b_26w_4s']), strict = False)
    return model
